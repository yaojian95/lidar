...
8. [硬件原理 (Hardware Principle)](#硬件原理)
...
### 线激光 + 双目相机 (Laser Line + Binocular Camera)

本系统软件层处理的输入端是 **点云 (Point Cloud)**，而在硬件采集端，其核心工作机理如下：

1.  **线激光投射 (Laser Line Projection)**:
    - 激光发生器向皮带上的矿石投射一条极细的结构光（线激光）。
    - 遇到不平整的矿石表面时，平直的激光线会产生几何畸变，这种畸变包含了深度信息。

2.  **双目影像捕捉 (Binocular Capture)**:
    - 左、右两个相机同步拍摄被激光照射的物体。
    - 由于位置差异，两个相机视角中的激光条纹中心在图像坐标系（$u, v$）下存在位移，即**视差 (Disparity)**。

3.  **条纹提取与立体匹配 (Stripe Extraction & Stereo Matching)**:
    - 系统首选从左、右图像中提取出激光条纹的子像素中心。
    - 进行立体匹配：寻找左图条纹上的点在右图中对应的位置。

4.  **深度计算 (Depth Calculation)**:
    - 利用相机标定得出的内参（焦距 $f$）和外参（基线 $B$），根据三角测量原理计算深度 $Z$:
      $$Z = \frac{f \cdot B}{d}$$
      其中 $d$ 为视差。
...
从底层数学计算顺序来看：
- **深度 (Depth/Disparity) 是最先被算出来的**。在每一个像素点位上，通过视差得到 $Z$ 值，这在逻辑上形成了一个 **深度图 (Depth Map)** 或深度剖面。
- **点云 (Point Cloud)** 是随后生成的。通过反向投影公式，将图像坐标系中的 $(u, v, Z)$ 转换到现实世界三维坐标系 $(X, Y, Z)$。

**总结**：在传感器底层的物理成像链路中，**深度 (Depth) 是因，点云 (Point Cloud) 是果**。硬件传感器（如线扫一体机）内部计算出深度 Profile 后，将其按标定的坐标映射输出为点云。

---

### 不同视角下的“先后顺序” (Perspective Matters)

用户有时会听到“先有点云再有深度图”的说法，这通常是在**软件系统应用层**的语境下：

1.  **硬件端视角 (Bottom-Up)**:
    - 过程：立体匹配 $\to$ 视差深度 $\to$ 三维重投影。
    - 结论：**深度在前，点云在后**。传感器必须先解出 Z 值，才能将物体“摆”在三维空间里。

2.  **软件/应用端视角 (Top-Down)**:
    - 过程：获取点云 (.pcd/.ply) $\to$ 对齐地球 $\to$ 正交投影 $\to$ 厚度图 (Thickness Map)。
    - 结论：**点云在前，深度图在后**。例如在本项目中，我们拿到的是已经成型的点云文件，通过软件算法将其投向 XY 平面来生成用于分析的“深度热力图”。

**一句话总结**：传感器制造者认为“深度生成点云”，而工业应用者通常认为“点云投影产生深度图（或厚度图）”。
...
- `belt_speed`: **皮带运行速度** (m/s)。用于精确计算点云在皮带运行方向上的步进间距。
- `scan_rate`: **激光扫描帧率** (Hz)。配合 `belt_speed` 计算纵向物理分辨率。
...
- `xray_high_energy_correction_factor`: **高能图像几何畸变校正系数**。由于高低能传感器高度不同（扇形投影），通过此系数对高能图像进行二次横向缩放以实现物理对准。
...
#### 5.6 体积计算逻辑 (Volume Calculation Logic)

**目的**: 实现基于 3D 点云原生累加的精确体积算法，避免 2D 插值带来的面积畸变。

**支撑参数**:
1.  **纵向步进 (dx)**: $dx = (BeltSpeed [m/s] \times 1000 [mm/m]) / ScanRate [Hz]$
2.  **横向步进 (dy)**: $dy = BeltWidth [mm] / (PointsPerLine - 1)$

**计算公式**:
- 单点体积贡献 ($v_i$): $v_i = thickness_i [mm] \times (dx\_mm [mm] \times dy\_mm [mm])$
- 总体积 ($V [cm^3]$): $V = (\sum v_i) / 1000.0$

**分辨率参数说明**:
系统不再通过皮带速度自动计算分辨率，而是直接使用 `config.yaml` 中的显式参数：
- **dx_mm**: 扫描线之间的纵向步进间距（沿着皮带运动方向）。
- **dy_mm**: 每行扫描点之间的横向间距（垂直于运动方向）。
这两个参数定义了体积积分时的微元底面积。

**输出对齐**: 
分析结果会自动保存至 `results/` 目录下。文件名与关联的 `xray_path` 保持一致，后缀为 **`.csv`**。

导出的文件采用了标准的 **CSV (逗号分隔)** 格式，确保了最大的软件兼容性。每轮分析的路径具有动态解析特性，会自动根据当前分辨率和检测模式生成版本后缀（例如 `results_0309_0p002_by_mask`）。此外，可以使用 `py_src/compare_volumes.py` 工具来自动化对比本系统计算的体积与排水法测得的参考数据。该工具目前支持通过**全局固定路径配置**直接指定 `REF_FILE` (参考 Excel) 与 `GEN_FILES` (生成的 CSV 列表)，并支持在同一张**合并横轴**的折线图中进行**多文件横向对比**。生成的对比图表 `volume_comparison.png` 会自动保存至第一个有效结果文件所在的目录中。

#### 5.5 坐标映射与转置逻辑 (Coordinate Mapping & Transpose)

**问: 为什么不直接将所有点云投影到 XY 平面？**

虽然 `thickness_map` 本质上是点云在 XY 平面的投影，但为了满足**视觉对齐**和**厚度量化**的需求，必须经过以下关键步骤：

1.  **地面对齐 (Ground Alignment)**: 必须先将点云旋转至地面法向量为 $(0,0,1)$ 且地面位于 $Z=0$。这样点云的 $Z$ 坐标才真正代表“厚度”。直接投影会因传感器倾斜导致厚度计算错误。
2.  **网格化与最大值池化 (Discretization & Max-Pooling)**: 图像是规则像素阵列，点云是离散点。在映射到像素时，如果一个像素内落入多个点，系统采用 **Max-Pooling**（取最大 $Z$ 值），确保捕捉到矿石的最顶端（最高厚度）。
3.  **坐标轴转置 (Axis Transpose)**: 
    - **LiDAR 坐标系**: $X$轴为皮带运行方向（长度），$Y$轴为皮带横向（宽度）。
    - **图像坐标系**: $Rows$ 通常对应图像的高度，$Cols$ 对应图像的宽度。
    - 为了让生成的厚度图在视觉上与相机图像（或人类直觉）一致（即皮带长度方向展现为图像的纵向），代码执行了 `cv::transpose`：
        - `Point.x` (Longitudinal) $\to$ `Image.row` (Vertical)
        - `Point.y` (Lateral) $\to$ `Image.col` (Horizontal)
4.  **填充与插值 (Interpolation)**: LiDAR 点云在远端或遮挡处非常稀疏，直接投影会有大量黑点。通过形态学闭运算或邻域插值，可以将零散的点连成连续的矿石形体。
...
### 7.4 `by_mask` 模式下的 05-08 图片生成全过程

在 `detection_mode: "mask"` (或 `by_mask`) 模式下，系统遵循“图像主导，点云逆映射”的逻辑，图片 05-08 的生成具有严格的先后顺序和坐标变换关系：

#### 1. `05_raw_thickness_map.tif` (原始物理底图)
- **生成时机**：在完成地面对齐后，系统率先生成一张覆盖当前 ROI 的全局厚度图。
- **核心逻辑**：
  - 调用 `generateGlobalThicknessMap`。
  - **坐标映射**：将 3D 点云 $(X, Y, Z)$ 投射到 2D 网格。
    - $Col = (X_{pcd} - min\_X) / resolution\_raw$
    - $Row = (max\_Y - Y_{pcd}) / resolution\_raw$
    - 这里的 $X_{pcd}$ 是皮带运行方向（长度），$Y_{pcd}$ 是跨皮带方向（宽度）。
  - **转置 (Transpose)**：为了符合视觉习惯（长度方向为纵向），代码在保存前执行了 `cv::transpose`。因此，在生成的图片中，**行(Row)** 对应点云的 $X$ 轴（长度），**列(Col)** 对应点云的 $Y$ 轴（宽度）。
- **属性**：32-bit float、灰度（数值即为厚度 mm）、原始物理分辨率。

#### 2. `06_rescaled_thickness_map.tif` (重采样对齐图)
- **生成时机**：将原始底图与视觉图像（XRT/RGB）进行 1:1 分辨率对齐。
- **核心逻辑**：
  - 调用 `alignThicknessMapToImage`。
  - 执行二次转置与 `cv::resize`，强行将厚度图的像素尺寸拉伸得与裁剪后的 XRT 图像完全一致。
- **属性**：32-bit float、分辨率与 XRT 图像一致。

#### 3. `07_ores_thickness_map.tif` (矿石纯净深度图)
- **生成时机**：利用视觉分割结果过滤背景。
- **核心逻辑**：
  - 从 XRT 图像中提取矿石轮廓 (Contours) 生成掩码。
  - 将 `06` 号图中所有不在矿石掩码区域内的像素全部置为 `0.0`。
- **属性**：32-bit float、仅包含矿石本体厚度、背景完全掏空。

#### 4. `08_fused_thickness_xray_byMask.jpg` (最终融合热力图)
- **生成时机**：视觉渲染输出。
- **核心逻辑**：
  - 将 `07` 号图的物理厚度值映射到 Jet 色阶（蓝 $\rightarrow$ 红）。
  - 与 XRT 灰度图进行 Alpha 融合。
  - 根据排序逻辑为每个矿石块标注 ID (0, 1, 2...)。
- **属性**：8-bit BGR 彩色、视觉直观、包含编号。

---

**最后更新**: 2026-04-08 08:30 (Antigravity)
