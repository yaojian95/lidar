...
导出的文件采用了标准的 **CSV (逗号分隔)** 格式，确保了最大的软件兼容性。每轮分析的路径具有动态解析特性，会自动根据当前分辨率和检测模式生成版本后缀（例如 `results_0309_0p002_by_mask`）。此外，可以使用 `py_src/compare_volumes.py` 工具来自动化对比本系统计算的体积与排水法测得的参考数据。该工具目前支持通过**全局固定路径配置**直接指定 `REF_FILE` (参考 Excel) 与 `GEN_FILES` (生成的 CSV 列表)，并支持在同一张**合并横轴**的折线图中进行**多文件横向对比**。
